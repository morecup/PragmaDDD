...
import org.morecup.pragmaddd.core.annotation.ListOrmFields
import org.morecup.pragmaddd.core.annotation.OrmField
import org.morecup.pragmaddd.core.annotation.OrmFields
import org.morecup.pragmaddd.core.annotation.OrmObject
import org.morecup.pragmaddd.core.annotation.PolyListOrmFields
import org.morecup.pragmaddd.core.annotation.PolyOrmFields
import org.morecup.pragmaddd.core.bridge.IConstructorBridge
import org.morecup.pragmaddd.core.bridge.IFieldBridge
    ...
    override fun getFieldValue(
        ...
        val result = when {
            ...
            listOrmFields!= null -> {
                ...
                TrackedAssociationList(
                    baseListOrmObj,
                    baseListOrmFieldList,
                    mappedList,
                    baseListOrmEntity
                )
                { index, addedDomainEntity ->
                    ...
                    val baseOrmObj = if (emptyNameIndex != -1) {
                        ...
                        // 实际实现可能需要根据具体业务需求调整
                        val baseListOrm =
                            OrmEntityOperatorConfig.operator.getEntityField(
                                baseListOrmObj,
                                baseListOrmFieldList
                            ) as List<Any>
                        // 获取list内实际的泛型
                        ...
                        // 创建一个新的ORM对象
                        OrmEntityConstructorConfig.constructor.createInstance(
                            baseListOrmGenericType as Class<*>
                        )
                    }
                    if (index >= 0) {
                        OrmEntityOperatorConfig.operator.addElementToEntityListAt(
                            baseListOrmObj,
                            baseListOrmFieldList,
                            index,
                            baseOrmObj
                        ...
                        OrmEntityOperatorConfig.operator.addElementToEntityList(
                            baseListOrmObj,
                            baseListOrmFieldList,
                            baseOrmObj
                        )
                    ...
                    withIndexCustomNames.forEach {
                        if (it.value.isNotBlank()) {
                            OrmEntityOperatorConfig.operator.setEntityField(
                                baseOrmObj,
                                it.value.split("."),
                                ormObjs[it.index]
                            )
            ...
            polyListOrmFields != null -> {
                ...
                { index, addedDomainEntity ->
                    ...
                    val withIndexCustomNames = polyListOrmFields.columnNames.mapIndexedNotNull { index, value ->
                        val columnName = value.columnName.ifEmpty {
                            value.columnChoiceNames[value.columnChoiceTypes.indexOf(addedDomainEntity::class)]
                        }
                        if (columnName.contains("base:")) {
